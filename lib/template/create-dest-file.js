 ...
 * @return {Promise<Void>}
 */
module.exports = function createDestFile (srcFileAbsPath, destFileAbsPath, dict, delimiter) {
...
function replacePlaceholders (dict, delimiter) {
  ...
  function flush (cb) {
    if (buf.length) this.push(Buffer.from(buf));
    this.end();
    cb();
  }
}
...
function processToken (buf, dict, delimiter) {
  const token = Buffer.from(buf).toString();
  ...
  return Buffer.from(subtokens.join(''));
}
